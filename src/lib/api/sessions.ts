...
class SessionsApiService extends BaseApiService {
  ...
  async registerMemberForSession(
    ...
    try {
      logger.info("Registering member for session", {
      ...
      if (!sessionResult.success || !sessionResult.data) {
        logger.warning("Session not found for registration", {
          sessionId: registrationData.sessionId,
        });
        return {
          data: null,
      ...
      if (session.registeredCount >= session.capacity) {
        logger.warning("Session is full", {
          sessionId: registrationData.sessionId,
      ...
      if (existingRegistration) {
        logger.warning("Member already registered for session", {
          sessionId: registrationData.sessionId,
          memberId: registrationData.memberId,
        });
        return {
      ...
      if (!memberResult.success || !memberResult.data) {
        logger.error(
          "Member not found for registration",
      ...
      if (
        ...
        const newRemainingClasses =
          member.remainingClasses === -1 ? -1 : member.remainingClasses - 1;
        await membersApi.updateMember(registrationData.memberId, {
          remainingClasses: newRemainingClasses,
        });
        logger.debug("Updated member remaining classes", {
          memberId: registrationData.memberId,
          newRemainingClasses,
        });
      }

      const result = await this.update<Session>(registrationData.sessionId, {
        registrations: updatedRegistrations,
        registeredCount: updatedRegisteredCount,
      });

      if (result.success) {
        logger.event("Session Registration", {
          sessionId: registrationData.sessionId,
          memberId: registrationData.memberId,
          memberName,
      ...
      logger.error("Error registering member for session", error as Error, {
        sessionId: registrationData.sessionId,
        memberId: registrationData.memberId,
      });
      return {
  ...
  async cancelRegistration(
    ...
    try {
      logger.info("Canceling session registration", { sessionId, memberId });

      const sessionResult = await this.getById<Session>(sessionId);

      if (!sessionResult.success || !sessionResult.data) {
        logger.warning("Session not found for cancellation", { sessionId });
        return {
          data: null,
          error: sessionResult.error || "Không tìm thấy ca tập",
          success: false,
      ...
      if (memberResult.success && memberResult.data) {
        ...
        if (
          ...
          logger.debug("Restored member remaining classes", {
            memberId,
            newRemainingClasses,
          });
        }
      ...
      const result = await this.update<Session>(sessionId, {
        registrations: updatedRegistrations,
        registeredCount: activeRegistrations.length,
      });

      if (result.success) {
        logger.event("Session Cancellation", {
          sessionId,
          memberId,
          sessionDate: session.date,
          sessionTime: session.startTime,
      ...
      logger.error("Error canceling session registration", error as Error, {
        sessionId,
        memberId,
      });
      return {
