...
class MembersApiService extends BaseApiService {
  ...
  async createMember(memberData: MemberCreateRequest): Promise<ApiResponse<Member>> {
    try {
      ...
      const result = await this.create<Member>(memberDoc);
      
      if (result.success && result.data) {
        // Update the user document with the Firebase Auth UID
        await this.update(userCredential.user.uid, { id: userCredential.user.uid });
      ...
      if (error && typeof error === 'object' && 'code' in error) {
        switch ((error as { code: string }).code) {
          ...
          default:
                    return {
          data: null,
          error: `Lỗi tạo tài khoản: ${(error as unknown as { message: string }).message}`,
          success: false,
        };
        }
      }
  ...
  async getAllMembers(filters?: MemberFilters): Promise<ApiResponse<Member[]>> {
    ...
    if (result.success && result.data && filters?.search) {
      ...
      result.data = result.data.filter(member => 
  ...
  async getMembersStats(): Promise<ApiResponse<{
    total: number;
    active: number;
    inactive: number;
    suspended: number;
    ...
    try {
      const [totalResult, activeResult, inactiveResult, suspendedResult] = await Promise.all([
        this.count(),
        this.count([{ field: 'membershipStatus', operator: '==', value: 'active' }]),
        this.count([{ field: 'membershipStatus', operator: '==', value: 'inactive' }]),
        this.count([{ field: 'membershipStatus', operator: '==', value: 'suspended' }]),
      ]);
      ...
      const newThisMonthResult = await this.count([
        { field: 'createdAt', operator: '>=', value: this.stringToTimestamp(startOfMonth.toISOString()) }
      ]);
